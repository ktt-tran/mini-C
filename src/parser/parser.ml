...
(* and parse_Call tokens =
  let t1, e1 = parse_PrimaryExpr tokens in
  match lookahead t1 with
  | Tok_ID c ->
    ...
    | Tok_LParen ->
      ...
      let t4, e2 = parse_Arg t3 in
      if lookahead t4 = Tok_RParen then
        (match_token t4 Tok_RParen, e2)
...
and parse_Comp tokens =
  match lookahead tokens with
	| Tok_Int_Type | Tok_Bool_Type | Tok_ID(_) | Tok_Assign | Tok_Print | Tok_If | Tok_For | Tok_While ->
...
and parse_compStmt tokens =
  ...
  | Tok_ID c ->
    let t, s = parse_Assign tokens in
		(t, s)
	(* | Tok_Assign ->
		let t1 = match_token tokens Tok_Assign in  
		let t2, s = parse_Assign t1 in
		(t2, s) *)
	| Tok_Print ->
...
and parse_stmt toks : token list * stmt =
  parse_Stmt toks;;
